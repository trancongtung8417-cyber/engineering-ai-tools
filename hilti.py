...
# --- 3. CSS GIAO DIỆN (MÀU ĐỎ HILTI, KHUNG VIỀN XÁM, & LAYOUT) ---
st.markdown("""
    <style>
    ...
    /* Cấu hình container chính cho phiếu */
    .receipt-main-container {
        padding: 40px; # Tăng padding để tạo khoảng cách
        background-color: #FFFFFF;
        margin-top: -30px; # Kéo lên trên cùng
        border: 2px solid #DD2222; # Giữ viền đỏ bên ngoài container
    ...
    /* Khung viền xám bo góc bao lấy tiêu đề */
    .receipt-header-box {
        border: 2px solid #808080; # Đổi màu viền thành xám
        padding: 15px;
    ...
    .receipt-header-text {
        color: #DD2222; # Giữ màu chữ tiêu đề đỏ
        margin: 0;
        font-size: 1.8rem;
    }

    /* Dòng kẻ xám giữa các mục thông tin */
    .receipt-info-separator {
        border-bottom: 1px solid #E0E0E0; # Dòng kẻ xám nhạt
        margin: 15px 0;
    }
...
def generate_pdf(data, timestamp):
    ...
    # 2. Khung viền xám bo góc bao lấy tiêu đề PDF
    pdf.set_y(15)
    
    # Thiết lập màu viền xám
    pdf.set_draw_color(128, 128, 128) # Đổi màu viền thành xám
    # Thiết lập màu chữ đỏ cho tiêu đề
    ...
    pdf.cell(box_w, box_h - 2, 'BIÊN BẢN NHẬN MÁY', ln=True, align='C')
    
    pdf.ln(10) # Khoảng cách xuống
    # Reset màu chữ về đen cho nội dung
    pdf.set_text_color(0, 0, 0) 
    pdf.set_font('Roboto', '', 12)
    
    # 3. Kẻ bảng nội dung (Kẻ dòng xám nhạt)
    col_label = 45
    col_value = 145 
    ...
    # Thiết lập màu dòng kẻ xám nhạt
...
if st.session_state['form_submitted']:
    ...
    # TẠO CONTAINER CHÍNH (Có viền đỏ bên ngoài)
    st.markdown('<div class="receipt-main-container">', unsafe_allow_html=True)
    
    # Logo Hilti lên trên cùng
    st.markdown(f'<div class="receipt-logo">', unsafe_allow_html=True)
    if os.path.exists(logo_path):
    ...
    st.markdown("<h1 class='receipt-header-text'>BIÊN BẢN NHẬN MÁY</h1>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    # Dòng kẻ xám separator trước tình trạng
    st.markdown('<div class="receipt-info-separator"></div>', unsafe_allow_html=True)

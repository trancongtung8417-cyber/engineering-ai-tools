...
if not st.session_state.logged_in:
    if SPA_LOGO: st.image(SPA_LOGO, width=150)
    ...
    if st.session_state.role == "Owner":
                    ...
                    if st.form_submit_button("Tạo ngay", use_container_width=True):
                        if not nu or not np: st.error("Vui lòng nhập Tên và Mật khẩu!")
                            ...
                            supabase.table("users").insert({
                                ...
                                "balance": 0, "status": "Active", "phone": n_phone, "note": n_note
                            }).execute()
                            st.rerun()
            with c2:
                all_u = get_all_users()
        ...
        if u_info:
            ...
            if balance < 100000:
                with st.warning("⚠️ **Số dư dưới 100.000 VND**"):
                    c1, c2 = st.columns([2, 1])
                    with c1: st.write(f"🔗 **Liên hệ nạp:** [Tại đây]({C_LINK})")
                    with c2: 
